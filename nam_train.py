""" Training script for Neural Additive Models. """
...
flags.DEFINE_string('logdir', 'log', 'Path to dir where to store summaries.')
flags.DEFINE_string('dataset_name', 'Adult', 'Name of the dataset to load for training.')
...
flags.DEFINE_integer('num_basis_functions', 1000,
...
flags.DEFINE_integer('max_checkpoints_to_keep', 1,
                     'Indicates the maximum number of recent checkpoint files to keep.')
flags.DEFINE_integer('save_checkpoint_every_n_epochs', 10,
                     'Indicates the number of epochs after which an checkpoint is saved')
...
flags.DEFINE_string('activation', 'exu',
                    'Activation function to used in the hidden layer. Possible options: (1) relu, (2) exu')
flags.DEFINE_boolean('regression', False,
                     'Boolean flag indicating whether we are solving a regression task or a classification task.')
...
@flags.multi_flags_validator(['data_split', 'cross_val'],
                             message='Data split should not be used in conjunction with cross validation')
...
def training(x_train, y_train, x_valid, y_valid, logdir):
  ...
  """
    ...
    batch_size = min(FLAGS.batch_size, x_train.shape[0])  # batch size cannot exceed training set size
    num_steps_per_epoch = x_train.shape[0] // batch_size  # number of steps per epoch
    ...
    # Boolean variables to indicate whether the training of a specific model has been early stopped.
    ...
    with tf.Graph().as_default():
        ...
        # Setup your training.
        graph_tensors_and_ops, metric_scores = _create_computation_graph(x_train, y_train, x_valid, y_valid, batch_size)
    ...
    for n in range(FLAGS.n_models):
        tf.logging.info(
            ...
            n, curr_best_epoch[n], metric_name, best_train_metric[n], best_validation_metric[n])
