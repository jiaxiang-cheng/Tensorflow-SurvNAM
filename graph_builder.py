...
def cross_entropy_loss(model, inputs, targets):
    ...
    loss_vals = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=logits)
    return tf.reduce_mean(loss_vals)


def penalized_loss(loss_func, model, inputs, targets, output_regularization, l2_regularization=0.0, use_dnn=False):
...
def penalized_cross_entropy_loss(model, inputs, targets, output_regularization, l2_regularization=0.0, use_dnn=False):
    """Cross entropy loss with L2 regularization and output penalty."""
    return penalized_loss(cross_entropy_loss, model, inputs, targets, output_regularization, l2_regularization, use_dnn)


def penalized_mse_loss(model, inputs, targets, output_regularization, l2_regularization=0.0, use_dnn=False):
    """Mean Squared Error with L2 regularization and output penalty."""
    return penalized_loss(mse_loss, model, inputs, targets, output_regularization, l2_regularization, use_dnn)
...
def feature_output_regularization(model, inputs):
    ...
    # L2 Regularization
    per_feature_norm = [tf.reduce_mean(tf.square(outputs)) for outputs in per_feature_outputs]
...
def create_iterators(datasets, batch_size):
    ...
    """
    tf_datasets = [tf.data.Dataset.from_tensor_slices(data).batch(batch_size) for data in datasets]
    input_iterator = tf.data.Iterator.from_structure(tf_datasets[0].output_types, tf_datasets[0].output_shapes)
...
def create_nam_model(x_train, dropout, feature_dropout=0.0, num_basis_functions=1000, units_multiplier=2,
                     activation='exu', name_scope='model', shallow=True, trainable=True):
...
def build_graph(
    ...
    x_batch, (train_init_op, test_init_op) = create_iterators((x_train, x_test), batch_size)
